...
module.exports = class Jnclude {
    ...
    execute (params) {
        // parsing options
        this._options = {develop: null, fullinfo: null, callback: null};
        // source file
        ...
        .on('data', (chunk) => {
            ...
            if (count) {
                timer = setTimeout (function tick () {
                    if (self._buffer.obj[src].read === fs.statSync(src).size
                            && i === count - 1) {
                        last_block = true;
                    ...
                    if (i === count ) {
                        self._buffer.obj[src].stream.resume();
                    } else {
                        self.checkDirectives(blocks, i, offset, last_block, eof)
                        .then(result => {
                            // end writing
        ...
        return new Promise ( (resolve, reject) => {
            ...
            if (find) {
                ...
                path = find[3]
                    ...
                    : null;
                flag = find[4]? find[4].split(' ') : [];

                full_offset = full_offset + offset;

                for (let i = flag.length; i--; ) {
                    if (this._flags[flag[i]]) {
                        flag_work = true;
                    };
                };

                //if flags not given ignore source flags
                if (!this._use_flags) {
                    flag_work = true;
                };

                if (com === 'include' && flag_work) {
                ...
                if (com === 'exclude' && !this._options.develop && flag_work) {
    ...
    showResult() {
        ...
        console.log(`${this._console.c_green}Jnclude done!${this._console.reset}`);
        if (this._options.fullinfo) {
    ...
    showError (key, addon = '') {
        ...
        error_list[202] = 'Destination file already exists in sourse path. Rename destination file or select other path';
